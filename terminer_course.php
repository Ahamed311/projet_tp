...
$req_en_cours = "SELECT c.courses_id, c.point_depart, c.point_darrivee, c.date_heure, c.image_vehicule,
                CONCAT(ch.prenom, ' ', ch.nom) AS chauffeurs
                FROM courses c
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="courses_id" required>
                    ...
                    <option value="<?php echo $course['courses_id']; ?>">
                        ...
                        | Chauffeur: <?php echo $course['chauffeurs']; ?>
                    </option>
                    <?php endwhile; ?>
                </select>
            </div>

            <button type="submit" class="btn btn-success" name="submit">Marquer comme Terminee</button>
        </form>
        <?php else: ?>
